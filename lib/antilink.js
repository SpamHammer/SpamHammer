...
async function Antilink(msg, sock) {
	...
	antilinkConfig = await getAntilink(jid, 'on');
		...
		switch (action) {
			case 'delete':
				await sock.sendMessage(jid, { 
					text: `\`\`\`@${sender.split('@')[0]} your message has been deleted because it contains one or more unwanted links.\`\`\``,
					mentions: [sender] 
				});
				break;
			...
			case 'kick':
				...
				await sock.sendMessage(jid, {
					text: `\`\`\`@${sender.split('@')[0]} has been kicked for sending unwanted links\`\`\``,
					mentions: [sender]
				});
				break;
			...
			case 'warn':
				...
				if (warningCount >= WARN_COUNT) {
					...
					await sock.sendMessage(jid, {
						text: `\`\`\`@${sender.split('@')[0]} warning ${warningCount}/${WARN_COUNT} for sending unwanted links\`\`\``,
						mentions: [sender]
					});
				}
				break;
