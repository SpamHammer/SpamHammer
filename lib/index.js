...
async function setAntilink(groupId, type, action = 'delete', mode = 'whitelist') {
    try {
        const data = loadUserGroupData();
        if (!data.antilink) data.antilink = {};
        if (!data.antilink[groupId]) data.antilink[groupId] = {};
        
        const existingConfig = data.antilink[groupId];
        data.antilink[groupId] = {
            enabled: type === 'on',
            action: action,
            mode: mode,
            ...(existingConfig?.allowedLinks && { allowedLinks: existingConfig.allowedLinks }),
            ...(existingConfig?.blockedLinks && { blockedLinks: existingConfig.blockedLinks })
        };
...
async function allowLinks(chatId, links) {
    try {
        const data = loadUserGroupData();
        if (!data.antilink) data.antilink = {};
        if (!data.antilink[chatId]) data.antilink[chatId] = {};

        if (!Array.isArray(data.antilink[chatId].allowedLinks)) {
            data.antilink[chatId].allowedLinks = [];
        }
        // Strip http:// , https:// , and www.  from links
        links = links.map(link => link.replace(/^(https?:\/\/)?(www\.)?/, ''));
        // Remove links from blockedLinks if they exist there
        if (Array.isArray(data.antilink[chatId].blockedLinks)) {
            data.antilink[chatId].blockedLinks = data.antilink[chatId].blockedLinks.filter(link => !links.includes(link));
        }
        data.antilink[chatId].allowedLinks.push(...links);

        saveUserGroupData(data);
        return true;
    } catch (error) {
        console.error('Error allowing links:', error);
        return false;
    }
}

async function blockLinks(chatId, links) {
    try {
        const data = loadUserGroupData();
        if (!data.antilink) data.antilink = {};
        if (!data.antilink[chatId]) data.antilink[chatId] = {};
        ...
        if (!Array.isArray(data.antilink[chatId].blockedLinks)) {
            data.antilink[chatId].blockedLinks = [];
        }
        // Strip http:// , https:// , and www.  from links
        links = links.map(link => link.replace(/^(https?:\/\/)?(www\.)?/, ''));
        // Remove links from allowedLinks if they exist there
        if (Array.isArray(data.antilink[chatId].allowedLinks)) {
            data.antilink[chatId].allowedLinks = data.antilink[chatId].allowedLinks.filter(link => !links.includes(link));
        }
        data.antilink[chatId].blockedLinks.push(...links);

        saveUserGroupData(data);
        return true;
    } catch (error) {
        console.error('Error blocking links:', error);
        return false;
    }
}

async function forgetLinks(chatId, links) {
    try {
        const data = loadUserGroupData();
        if (!data.antilink || !data.antilink[chatId]) return false;
        ...
        console.error('Error forgetting links:', error);
        return false;
    }
}

async function resetAntilinkSettings(chatId) {
    try {
        const data = loadUserGroupData();
        if (data.antilink && data.antilink[chatId]) {
            delete data.antilink[chatId];
            saveUserGroupData(data);
        }
        return true;
    } catch (error) {
        console.error('Error resetting antilink settings:', error);
        return false;
    }
}
